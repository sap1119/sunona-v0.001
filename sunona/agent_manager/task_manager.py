...
from sunona.constants import ACCIDENTAL_INTERRUPTION_PHRASES, DEFAULT_USER_ONLINE_MESSAGE, DEFAULT_USER_ONLINE_MESSAGE_TRIGGER_DURATION, FILLER_DICT, DEFAULT_LANGUAGE_CODE, DEFAULT_TIMEZONE
from sunona.helpers.function_calling_helpers import trigger_api, computed_api_response
from sunona.memory.cache.vector_cache import VectorCache
from .base_manager import BaseManager
from sunona.agent_types import *
from sunona.providers import *
from sunona.prompts import *
from sunona.helpers.utils import structure_system_prompt, compute_function_pre_call_message, get_date_time_from_timezone, get_route_info, calculate_audio_duration, create_ws_data_packet, get_file_names_in_directory, get_raw_audio_bytes, is_valid_md5, \
...
class TaskManager(BaseManager):
    def __init__(self, assistant_name, task_id, task, ws, input_parameters=None, context_data=None,
        ...
        if task_id == 0:
            ...
            self.synthesizer_voice = provider_config.get("voice", "default")
        ...
        if self.conversation_config is not None:
            ...
            self.minimum_wait_duration = self.task_config["tools_config"]["transcriber"].get("endpointing", 400)
    ...
    def __prefill_prompts(self, task, prompt, task_type):
        ...
        if not prompt:
            if task_type in ('extraction', 'summarization'):
                ...
                llm_config = task.get("tools_config", {}).get("llm_agent", {}).get("llm_config", {})
                if isinstance(llm_config, dict):
                    p = llm_config.get("prompt") or llm_config.get("system_prompt")
                    if p:
