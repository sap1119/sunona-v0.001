...
from sunona.helpers.utils import create_ws_data_packet, timestamp_ms
        ...
        try:
            while True:
                ...
                if ws_data_packet is not None:
                    ...
                    # Track STT usage (HTTP)
                    try:
                        from sunona.helpers.call_tracker import get_current_tracker
                        tracker = get_current_tracker()
                        if tracker:
                            tracker.track_stt_usage(transcription['meta_info']['transcriber_duration'])
                    except Exception as e:
        ...
        async for msg in ws:
            try:
                ...
                if message_type == "Begin":
                    ...
                    # Track STT usage (Streaming)
                    try:
                        from sunona.helpers.call_tracker import get_current_tracker
                        tracker = get_current_tracker()
                        if tracker:
                            tracker.track_stt_usage(audio_duration)
                    except Exception as e:
                        logger.warning(f"Failed to track STT usage: {e}")
